...
public class TextLineNumber extends JPanel
    ...
    public void setCompileInfo(final Map<Integer, List<CompileInfo>> lstInfo) {
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        if (lstInfo != null) {
            int rowStartOffset = component.viewToModel(new Point(0, y));
            Element root = component.getDocument().getDefaultRootElement();
            int index = root.getElementIndex(rowStartOffset);

            List<CompileInfo> lst = this.lstInfo.get(index + 1);
            if (lst != null) {
                txt += "<html>";
    ...
    public void paintComponent(Graphics g) {
        ...
        Element root = component.getDocument().getDefaultRootElement();
        int idCaret = root.getElementIndex(caretPosition);
...
//            lstInfoPopup.clear();
        while (rowStartOffset <= endOffset) {
            try {
                ...
                String lineNumber = (line.getStartOffset() == rowStartOffset) ? String.valueOf(index + 1) : null;

                if (lineNumber != null) {
                    int y = getOffsetY(rowStartOffset, fontMetrics);

                    List<CompileInfo> info = lstInfo == null ? null : lstInfo.get(index + 1);
                    if (info != null) {
                        int kind = info.get(0).kind;
                        Color c = (kind == 0) ? new Color(255, 100, 100) : (kind == 1) ? new Color(255, 165, 50) : new Color(100, 100, 255);
                        int x = insets.left + availableWidth - 10;//availableWidth/2-6;
                        g2.setPaint(new GradientPaint(x, y - 10, Color.white, x, y - 5, c));
